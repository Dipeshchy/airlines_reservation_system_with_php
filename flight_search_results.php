...
<div class="col-lg-8">
 ...
 <div class="col-lg-7 card mb-2 mt-5">
    <h2 class="text-center mt-2 text-primary">Searched Flights Results</h2>
    <hr>
    ...
    <div>
       <?php 
       		...
       		{
       			$search_starting_airport = $_POST['starting_airort'];
       		    $search_destination = $_POST['destination'];
       			$search_date = $_POST['search_date'];

       			$query_for_route = execute_query("SELECT * FROM airlines_reservation_system.routes WHERE starting_airport='{$search_starting_airport}' AND destination='{$search_destination}' ");
       			confirm($query_for_route);
       			
       		

       			while($rowroute=fetch_array($query_for_route))
       			{
       			 $search_route_id = $rowroute['route_id'];	


        $query = execute_query("SELECT * FROM airlines_reservation_system.flights WHERE route_id={$search_route_id} AND flight_date='{$search_date}' ");
        confirm($query); 
         $count = mysqli_num_rows($query);
        
         if($count == 0)
         {
         	echo "<h3 class='text-danger'>No Flights Available</h3>";
         }
         else
         {
